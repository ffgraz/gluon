...
class Node():
    ...
    Creating one only declares it. Everything a scenario asks of a node
    before :func:`start` - mesh links, uci settings, the domain - is
    recorded and applied while the guest is configured; everything
    after runs on the booted node over SSH.

    Each node also carries a ``dbg`` callable that writes a timestamped
    line to its log, which is archived with the run.
    """
    ...
    def add_mesh_link(self, peer, _is_peer=False, _port=None):
        ...
        Called by :func:`connect`; a scenario builds its topology with
        that, or with the topology helpers in :mod:`meshlib`."""
        self.if_index_max += 1
    ...
    def uci_set(self, config, section, option, value):
        """Set a uci option on the node before it is configured, so it
        is in place by the time gluon-reconfigure runs."""
    ...
    def if_client(self):
        ...
        # slot too: two runs side by side would otherwise both want to
        # create the same interface.
        return "client%d_%d" % (SLOT, self.id)

    def execute_in_background(self, cmd, _msg=True):
        """Start a command on the node and return without waiting.

        The returned handle has a ``cancel()`` that interrupts the
        command and waits for it to end - for things that have to keep
        running while the scenario does something else."""
        class bg_cmd:
    ...
    def succeed(self, cmd):
        ...
        A non-zero exit raises, with the output written to the node log
        first, so a failing scenario says what the node answered."""
    ...
    def wait_until_succeeds(self, cmd, timeout=180):
        """Run a command once a second until it exits zero, and return
        its output.

        The timeout is wall-clock. This is how a scenario waits for
        something to converge: state the condition as a command that
        fails until it holds."""
        output = ""
...
def start():
    """Boot every node declared so far and configure it.

    Returns once each node has been given its interface roles, uci
    settings and SSH key, has run gluon-reconfigure and is reachable -
    so a scenario declares its topology, calls this, and from then on
    talks to running nodes."""
    global workdir
    global configured
    if configured:
        return
    ...
    if not args.run_forever:
        # Registered here rather than called from finish(), because a
        # scenario that fails never reaches finish() and would leave
        # its image copies and ssh key behind. atexit unwinds in
        # reverse, so this runs after the qemus have been terminated.
...
def discard_artifacts():
    """Remove what only the running nodes needed: the image copy each
    was booted from, the ssh key they were handed and the symlinks to
    their consoles. The logs are what is left to look at afterwards,
    so they stay."""
    for d in ('images', 'ssh', 'ptys'):
        shutil.rmtree(os.path.join(workdir, d), ignore_errors=True)

def finish():
    """End the scenario and shut the nodes down.

    With ``--run-forever`` the nodes are left up instead, so a failing
    scenario can be inspected by hand - and so are the images, ssh key
    and console symlinks that a run otherwise discards on the way
    out."""
    if args.run_tests_on_existing_instance:
        return
...
def connect(a, b):
    """Put a mesh link between two nodes.

    Both get an interface on a segment only they share, so a topology
    is built by choosing which pairs to connect."""
    a.add_mesh_link(b)
