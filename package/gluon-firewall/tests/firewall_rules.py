...
"""Characterizes the client-bridge firewall behaviour the nftables
filters implement: that the rules a client can push against are
installed, and that the mesh still works with all of them loaded.
...
# The dump is archived in the node log for inspection.
...
assert 'nd-router-solicit' in dump, 'no RS direction rule'
assert 'nd-router-advert' in dump, 'no RA direction rule'
